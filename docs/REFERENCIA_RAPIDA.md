...
*Para consulta rápida al inicio de cualquier conversación · actualizado 23 mayo 2026 (hover de la tarjeta de partida completa restaurado en portátiles táctiles quitando el gate `any-hover`; antes ese día: conversión HEIC para móviles Samsung con heic-to; 22 mayo: timeout de subida de foto)*
...
✅ **Hover de la tarjeta de partida completa restaurado en portátiles táctiles (PR #38, commit `03c90d9`)**: el lift de `.card` dependía de `@media (any-hover: hover)`, que en algunos portátiles táctiles/híbridos se evalúa `false` pese a tener trackpad → la tarjeta se quedaba plana. El botón "Regalar" engañaba porque su lift viene de la regla **global** `.btn:hover` (sin media query). Se quita el gate y `.card:hover` se aplica siempre, igual que `.btn:hover`. Solo cambia el hover de la tarjeta; no toca acordeón, layout, botón ni lógica. Peaje aceptado: posible sticky-hover en móviles táctiles, idéntico al del botón.
